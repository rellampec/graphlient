...
- [Usage](#usage)
  ...
  - [Fragment Spreads in the DSL](#fragment-spreads-in-the-dsl)
  - [Directives in the DSL](#directives-in-the-dsl-coming-in-a-future-release)
...
### Fragment Spreads in the DSL

When building queries with a block, you can use `spread` to insert a named fragment
spread (`...FragmentName`) without switching to raw string interpolation:
...
This is useful when the fragment definition is managed externally — for example,
by a framework layer that appends fragment strings to the query before sending it
to the server, or when you are assembling queries programmatically and want to keep
the DSL clean.

**Defining fragments alongside the query (proposed — coming in a future release)**

A companion `fragment` DSL method is planned that lets you define fragment bodies
inline in the same block, so graphlient can assemble the complete query string
without any external tooling:

```ruby
response = client.query do
  # Define the fragment inline — body generated by the same DSL
...
Fragments are scoped to the query call — no global registry, no cross-contamination
between requests. Multiple fragments and nested spreads are supported.
...
### Directives in the DSL (coming in a future release)

Support for inline directive syntax in DSL blocks is planned, allowing expressions
like the following without falling back to raw strings:

```ruby
client.query(some_id: 42, skip_fee: true) do
